...
class API_Handler(object):
    ...
    def __init__(self, Api_Log_Path, limit):
        self.limit_quota = limit
        self.api_path = "{0}/api_keys.json".format(Api_Log_Path)  # holds API key-quota file path
        with open(self.api_path) as json_file:  # loading the API key-quota file
    ...
    def func_handler(self, api_key, ip_address):
        ...
        elif api_key not in self.api_file.keys():  # check if the key is registered in API key-quota file
            return "API key is not authorized."
        elif self.api_file.get(api_key).get(
                'quota') >= self.limit_quota:  # check if the specific api key quota is not exceeded
            return "API quota exceeded."
        else:
            return True

    def update_api_file(self):  # update the API key-quota file
