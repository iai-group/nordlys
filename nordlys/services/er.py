...
from nordlys.logic.er.elr import ELR
...
class ER(object):
    ...
    def __check_config(config):
        ...
        if config.get("model", None) is None:
            config["first_pass"]["num_docs"] = int(config["num_docs"])
    ...
    def retrieve(self, query):
        ...
        # for en_id, en in sorted(ens.items(), key=lambda item: item[1]["score"], reverse=True):
        #     if i < self.__start:
        #         continue
        #     rank = i + self.__start
        #     sorted_ens[rank] = {"entity": en_id, "score": en["score"]}
        #     if en.get("fields", {}) != {}:
        #         sorted_ens[rank]["fields"] = en["fields"]
        #     i += 1
